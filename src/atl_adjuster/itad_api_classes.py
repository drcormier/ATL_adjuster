"""
This module contains the underlying classes used to interact with the ITAD API, including:
...
class APIGetLowest(ABC):
    ...
    def get_lowest_price_for_game(self, plain_name: str, lowest: Lowest) -> float:
        return lowest.data[plain_name].price

    def get_lowest_price_for_games(self, plain_names: Iterable[str], lowest: Lowest) -> Iterable[float]:
...
class APIGetPlain(ABC):
    ...
    """
    id_pattern = re.compile(r'^https://store\.steampowered\.com/(\w+?/\d+?)/\w+/?$')
    ...
    def get_plain_name_for_game_by_steam_url(self, game_url: str) -> str:
        """
        Converts a Steam URL to a Steam ID and then calls get_plain_name_for_game_by_steam_id
        """
        return self.get_plain_name_for_game_by_steam_id(self.steam_url_to_steam_id(game_url))
    ...
    def steam_url_to_steam_id(self, steam_url: str) -> str:
        ...
        """
        return self.id_pattern.match(steam_url).group(1)
